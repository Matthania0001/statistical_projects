...
df_clean <- df %>% drop_na() # Suppression des lignes contenant des valeurs manquantes
...
IQR <- Q3 - Q1  
df_wt_outliers <- df_clean %>% filter(DAM >= Q1 - 1.5*IQR & DAM <= Q3 + 1.5*IQR)  
#print(df_wt_outliers) 
#Creation de variables dérivées
df_add_variables <- df_wt_outliers %>%  
  mutate(  
    Dépenses_par_personne = as.numeric(DAM) / as.numeric(Taille_agregée),  
    Catégorie_âge = cut(as.numeric(Age_CM), breaks = c(20, 40, 60, 80), labels = c("Jeune", "Adulte", "Âgé"))  
  )  

#exportation des données nettoyées avec les nouvelles variables 
write_xlsx(
  list("Feuille1" = df_add_variables),
  "../Data/donnees_multiples.xlsx"
)
